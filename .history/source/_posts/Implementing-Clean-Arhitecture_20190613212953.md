...
The implementations will be passed in the constructor as well as other 3rd party dependencies/modules. If we used/imported the dependencies/implementations directly, we would not adhere to the `Clean Architecture` dependency rule as we would create a dependency of a higher level module to a lower level detail which is against the `inwards dependency`, also called `dependency inversion`, arrows we saw in the diagram above. 
...
class ConcreteAddToCart extends AddToCart {
  async receiveFileFromCli(commandLineOptions){
    if(commandLineOptions && commandLineOptions["product"]){
      const isAdded = await this.add(commandLineOptions["product"])
...
class ConcreteRemoveFromCart extends RemoveFromCart {
  async removeFileFromCli(commandLineOptions){
    if(commandLineOptions && commandLineOptions["product"]){
      const isRemoved = await this.remove(commandLineOptions["product"])
      console.log(`isRemoved: ${isAdded}`)
    } else {
      console.error("commandLineOptions require product to be passed")
    }
  }
