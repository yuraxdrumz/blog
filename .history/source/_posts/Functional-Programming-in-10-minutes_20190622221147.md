...
## Pipes
Lets see how we put to practice all the above. In functional languages we usually have a pipe operator that allows an argument to pass through a pipe of function. In Math, it looks like this: `f(g(h(x, y)))`, and it Javascript it looks like this `const operate = (x, y) => square(addOne(multiply(x, y)))`