...
In Math, it looks like this: `f(g(h(x, y)))`.

In Javascript it looks like this `const operate = (x, y) => square(addOne(multiply(x, y)))`.
The above is not so user friendly, that is why we have pipes:
